...
// Python venv interpreter — created by setup.bat / setup.sh
const IS_WIN     = process.platform === 'win32';
const PYTHON_EXE = IS_WIN
    ? path.join(__dirname, 'server_env', 'Scripts', 'python.exe')
    : path.join(__dirname, 'server_env', 'bin', 'python');
...
const DEFAULT_EMBODY_EXE = null; // User selects on first launch

function getEmbodyExePath() {
    return store.get('embodyExePath', DEFAULT_EMBODY_EXE);
}
...
function killProc(proc) {
    ...
    // UE games ignore SIGTERM and spawn child processes that outlive the parent.
    // Force-kill the whole process tree on both platforms.
    if (pid) {
        if (IS_WIN) {
...
// Last-resort cleanup: catch process-level signals so the children
// are always terminated even if Electron exits unexpectedly.
...
// Wait until UE has connected as a streamer (endpointIdConfirm in signalling stdout).
// Resolves when connected or after timeoutMs — never rejects.
...
function startAnimServer() {
    // Skip silently if setup.bat hasn't been run (no venv yet)
    if (!fs.existsSync(PYTHON_EXE) || !fs.existsSync(SERVER_PY)) {
        console.log('[Server] Python venv or server.py not found — skipping animation server.');
        return;
    }

    serverProc = spawn(PYTHON_EXE, [SERVER_PY], {
        cwd: __dirname,
        stdio: 'pipe',
        env: { ...process.env, PYTHONUNBUFFERED: '1' },
    });

    serverProc.stdout.on('data', d => console.log('[AnimServer]', d.toString().trim()));
    serverProc.stderr.on('data', d => console.log('[AnimServer]', d.toString().trim()));
    serverProc.on('exit', code => {
        console.log('[AnimServer] exited with code', code);
        serverProc = null;
    });

    console.log(`[Main] Animation server started (PID ${serverProc.pid})`);
}
...
function createWindow() {
    mainWindow = new BrowserWindow({
        ...
        // Frameless with custom title bar if desired; for now keep native
        show: false,
    });
    ...
    mainWindow.loadURL(`http://localhost:${PLAYER_PORT}/mannequin.html`);
}
...
function showSetupWindow(errorMsg) {
    const setupWin = new BrowserWindow({
        width: 560,
        height: 340,
        title: 'Embody Mannequin Studio — Setup',
        backgroundColor: '#0a0a0a',
        resizable: false,
        webPreferences: {
            preload: path.join(__dirname, 'preload.js'),
...
// Save animation JSON to disk
ipcMain.handle('save-animation', async (_event, { defaultName, data }) => {
    const { filePath, canceled } = await dialog.showSaveDialog(mainWindow, {
        title: 'Save Animation',
        defaultPath: defaultName ? `${defaultName}.emanim` : 'animation.emanim',
        filters: [
            { name: 'Embody Animation', extensions: ['emanim'] },
...
// Load animation JSON from disk
ipcMain.handle('load-animation', async () => {
...
ipcMain.handle('browse-for-exe', async () => {
    ...
    // Close setup windows and start the real app
...
// Restart Embody.exe (useful after crash or when user wants a fresh session)
